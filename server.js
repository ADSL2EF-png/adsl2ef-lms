...
async function handleRegister(request, response) {
  ...
  const role = "student"; // rôle forcé à student pour les inscriptions publiques
  ...
  try {
    ...
    if (!signupRes.ok) {
      if (signupData.msg?.includes("already") || signupData.code === "email_exists") {
        json(response, 409, { error: "email_exists", message: "Cet email existe déjà." });
        return;
      }
    ...
    json(response, 201, { message: "Inscription reçue. En attente de validation.", userId: signupData.user?.id });
  } catch (err) {
...
async function handleAdminCreateUser(request, response) {
  ...
  try {
    ...
    const createRes = await fetch(`${SUPABASE_URL}/auth/v1/admin/users`, {
      ...
      body: JSON.stringify({
        email, password,
        email_confirm: true, // approuvé directement
        user_metadata: { name, role }
      })
    });
    const userData = await createRes.json();
    if (!createRes.ok) {
      json(response, 400, { error: "create_failed", message: userData.msg || "Création impossible." });
      return;
    }

    // Mettre à jour le profil avec le rôle et statut approuvé
    await supabaseFetch(`/profiles?id=eq.${userData.id}`, {
      method: "PATCH",
      prefer: "return=minimal",
      headers: { "Prefer": "return=minimal" },
      body: JSON.stringify({ name, role, approval_status: "approved", updated_at: new Date().toISOString() })
    });

    json(response, 201, {
      user: { id: userData.id, name, email, role, approvalStatus: "approved" }
    });
  } catch (err) {
