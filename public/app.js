...
async function handleAdminUserCreate(event) {
  ...
  const createdUser = {
    ...
    name,
    email,
    password: plainPassword, // envoyé à Railway qui hashera avec scrypt
    role: String(formData.get("role")),
    bio: "Compte cree par l'administration.",
    avatar: initials(name),
    createdAt: nowISO(),
    approvalStatus: "approved" // compte admin = approuvé directement
  };
  state.users.push(createdUser);
  addLog(getCurrentUser().id, `Utilisateur créé - ${name}`);
  // Envoyer à Railway via événement — Railway hashera le mot de passe avec scrypt
  ...
  // Remplacer le mot de passe en clair par un hash local pour le localStorage
  createdUser.password = await secureLocalPassword(plainPassword);
  closeModal();
  saveState();
}
