...
function getApiHeaders(useApiToken = false) {
  ...
  // Pour le sync d'état (PUT /lms/state, GET /lms/state) → token API
  // Pour les actions utilisateur → JWT de session
  const bearerToken = useApiToken
    ? persistence.apiToken
    : (state.session?.accessToken || persistence.apiToken);
...
async function loginWithApi(email, password) {
  const persistence = getPersistenceConfig();
  const response = await fetch(buildApiUrl(persistence.apiBaseUrl, persistence.authLoginPath), {
    method: "POST",
...
async function loadStateFromApi() {
  ...
  try {
    ...
    try {
      const usersRes = await fetch(buildApiUrl(persistence.apiBaseUrl, "/auth/users"), {
        headers: getApiHeaders(true)
      });
      if (usersRes.ok) {
      ...
      console.warn("Users load ignored:", e);
    }
  ...
  return `
    ...
    <section class="storefront-hero ${isSchool ? "storefront-school" : "storefront-pro"}">
      ...
      <div class="storefront-aside">
        ...
        ${isSchool ? `<div class="stat-pill"><strong>Classes + examens</strong><span>parcours ciblés</span></div>` : `<div class="stat-pill"><strong>CAT 1 à CAT 5</strong><span>parcours classés</span></div>`}
...
async function approveUser(userId) {
  ...
  const persistence = getPersistenceConfig();
  try {
    await fetch(buildApiUrl(persistence.apiBaseUrl, "/auth/approve"), {
      method: "POST",
      headers: getApiHeaders(true),
  ...
  try {
    ...
    if (!user && !shouldUseSupabasePersistence() && shouldUseApiPersistence()) {
      try {
        ...
        if (msg.includes("403") || msg.includes("pending_approval")) {
          alert("Votre compte est en attente de validation par l'administrateur ADSL-2EF. Vous serez notifié dès que votre accès sera activé.");
        } else if (msg.includes("rejected")) {
          alert("Votre demande d'accès a été refusée. Contactez l'administration ADSL-2EF.");
        } else {
    ...
    if (!user && !shouldUseSupabasePersistence() && shouldUseApiPersistence()) {
      try {
        ...
        if (error.message === "email_exists") {
          ...
          alert("Inscription impossible pour le moment. Veuillez réessayer.");
        }
        return;
      }
    }
...
async function handleAdminUserCreate(event) {
  ...
  if (shouldUseApiPersistence() && persistence.apiBaseUrl) {
    ...
    try {
      ...
      if (!res.ok) {
        alert(data.message || "Création impossible.");
        return;
      }
      // Ajouter localement aussi
      const newUser = { ...data.user, createdAt: nowISO() };
      ...
      console.warn("Admin create via API failed:", err);
    }
  }
